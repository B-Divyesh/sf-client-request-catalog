...
function renderOwner(data: OwnerData) {
  ...
  for (const [id, endpoint, name] of [['csv', '/api/admin/requests.csv', 'client-requests.csv'], ['pdf', '/api/admin/requests.pdf', 'client-requests.pdf']] as const) {
    ($(`#${id}`) as HTMLAnchorElement).addEventListener('click', async event => { event.preventDefault(); const response = await api(endpoint); if (response.ok) download(response, name); else window.alert('The export could not be created. Try again.'); });
  }
  ...
  document.querySelectorAll<HTMLFormElement>('.offer-assignment-form').forEach(form => form.addEventListener('submit', async event => { event.preventDefault(); const product_ids = [...form.querySelectorAll<HTMLInputElement>('input[name="offer_ids"]:checked')].map(input => Number(input.value)); const response = await api(`/api/admin/clients/${form.dataset.client}`, { method: 'PATCH', body: JSON.stringify({ product_ids }) }); const message = form.querySelector<HTMLElement>('.form-message')!; message.textContent = response.ok ? 'Visible offers saved.' : 'The visible offers could not be saved.'; if (response.ok) window.setTimeout(() => void loadOwner(), 350); }));
  $<HTMLFormElement>('#client-form').addEventListener('submit', async event => { event.preventDefault(); const form = event.currentTarget as HTMLFormElement; const fields = new FormData(form); const body = { name: String(fields.get('name')), expires_in_days: Number(fields.get('expires_in_days')), offer_ids: [...form.querySelectorAll<HTMLInputElement>('input[name="offer_ids"]:checked')].map(input => Number(input.value)) }; const response = await api('/api/admin/clients', { method: 'POST', body: JSON.stringify(body) }); if (response.ok) await loadOwner(); else $('#client-message').textContent = 'The private link could not be created. Check the name, expiry, and offers.'; });
  $('#product-form').addEventListener('submit', async event => { event.preventDefault(); const form = event.currentTarget as HTMLFormElement; const response = await api('/api/admin/products', { method: 'POST', body: JSON.stringify(Object.fromEntries(new FormData(form))) }); $('#product-message').textContent = response.ok ? 'Offer added. Assign it to a client link before sharing it.' : 'The offer could not be added. Check each field.'; if (response.ok) window.setTimeout(() => void loadOwner(), 700); });
  document.querySelectorAll<HTMLAnchorElement>('.export-request').forEach(link => link.addEventListener('click', async event => { event.preventDefault(); const response = await api(link.href); if (response.ok) download(response, 'request-export.csv'); else window.alert('The request export could not be created. Try again.'); }));
  document.querySelectorAll<HTMLButtonElement>('.delete-request').forEach(button => button.addEventListener('click', async () => { if (!window.confirm(`Delete request ${button.dataset.reference}? This removes its contact details and selected offers. The audit record keeps only an internal ID, action, and date.`)) return; const response = await api(`/api/admin/requests/${button.dataset.request}`, { method: 'DELETE' }); if (response.ok) await loadOwner(); else window.alert('The request could not be deleted. Try again.'); }));
